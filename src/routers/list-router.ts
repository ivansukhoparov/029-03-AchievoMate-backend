...
import {accessRightMiddleware, authMiddleware} from "../middlewares/auth-middleware";
...
listRouter.post("/:id/items", authMiddleware,accessRightMiddleware, async (req:Request, res:Response)=>{
    ...
    res.status(HTTP_STATUSES.CREATED_201).json(updatedList)
})
